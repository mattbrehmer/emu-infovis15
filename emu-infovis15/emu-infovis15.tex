...
Now lets characterize these domain problems in terms of data and task abstractions.
...
I'll begin with characterizing the data.
...
{\bf Predicted energy behaviour} based on statistical models are also considered, though these are problematic for reasons I'll describe in a few slides.
...
Most of the designs in the following slides were produced within this interactive environment, which was developed using RStudio's Shiny framework\footnote{\url{shiny.rstudio.com}}.
...
It is also accessible publicly on shinyapps.io\footnote{\url{mattbrehmer.shinyapps.io/PortfolioSandbox} \\username: {\tt ieeevis}; password: {\tt vis2015}}, though we had to use a small anonymized dataset for the latter due to client privacy concerns.
...
Since the space of possible techniques that we know of is large, we considered a few of them, implemented a subset of these, and ultimately selected a few good matches based on evaluation that coincided with design.
...
However, we found that most people we spoke to, both employees and clients, weren't familiar with boxplots, except for a minority who had taken a postsecondary stats class. 
...
Furthermore, the line chart encoding is already quite familiar to the energy workers that we interviewed.
...
Most of the energy workers that we interviewed were unfamiliar with this form of encoding, except one energy analyst who routinely made such visualizations in {\it Excel}. 
...
The calendar layout is familiar and allows us to show data at finer granularities than months, so this was very well-received by those we interviewed and served to reduce the unfamiliarity of the matrix layout.
...
Though the issue of boxplot familiarity is unresolved, these boxplots are easier to read than faceted boxplots, and they're reinforced by data shown the matrix, which is preferable to showing boxplots in isolation. 
...
Considering these tasks ({\bf T1--T3}), these five matches serve as generalizable guidelines that may apply beyond the energy domain.
...
Finally, they provided the ability to compare observed values against trusted historical values, instead of or in addition to comparing observed values to less trusted predicted values generated by a statistical model.
...
One collaborator remarked that we confirmed their earlier suspicions of what was wrong with {\it Energy Manager}, that we performed an analysis of [{\it Energy Manager}'s] flaws in a systematic way, put a name on them, and then tested with users.
...
In future work, we would like to assess the adoption of the redesigned Energy Manager, tracking usage over an extended period of time and following-up later with additional interviews and focus groups.
...
We described generalizable visualization design choices framed in terms of {\bf matches} and {\bf mismatches} that may apply beyond the energy analysis domain, for three tasks ({\bf T1--T3}) and a multidimensional table involving multiple time series, as well as more general guidelines pertaining to {\bf familiarity} and {\bf trust}.
